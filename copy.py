...
#     for file in files:
#         match = re.search('^[A-Z]\d\d-\d\d', file)
#         if match:
#             os.rename(file, 'AQDT '+ file)
def find_wb_main():
    for root, dirs, files in os.walk(os.getcwd()):
        for file in files:
            match = re.search('irSummary.xlsx', file)
            if match:
              print('match')
              return load_workbook(file)


def copy_wbs(main_wb):
  ...
  for root, dirs, files in os.walk(os.getcwd()+'/input/'):
    for file in files:
      ...
      if match:
        print('match')
        fn = file[:4] + file[27:36]
        wb = load_workbook(os.getcwd()+'/input/'+file)

        wb_target = main_wb
        target_sheet = wb_target.create_sheet(fn)

        wb_source = load_workbook(os.getcwd()+'/input/'+file, data_only = True)
